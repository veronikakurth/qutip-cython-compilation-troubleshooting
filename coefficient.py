...
def compile_code(code, file_name, parsed, c_opt):
    ...
    try:
        ...
        try:
            ...
            import Cython.Compiler.Main as CCM

            _orig_search = CCM.search_include_directories

            def traced_search_include_directories(*args, **kwargs):
                print("\n[Cython search_include_directories]")

                if len(args) >= 3:
                    dirs = args[0]
                    qualified_name = args[1]
                    suffix = args[2]
                ...
                if "source_file_path" in kwargs:
                    print("  source_file_path:", kwargs["source_file_path"])
                if "sys_path" in kwargs:
                    print("  sys_path:", kwargs["sys_path"])

                result = _orig_search(*args, **kwargs)

                print("  -> result:", result)
                return result

            CCM.search_include_directories = traced_search_include_directories
